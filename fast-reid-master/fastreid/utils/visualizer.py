...
class Visualizer:
    ...
    def save_rank_result(self, query_indices, output, max_rank=5, vis_label=False, label_sort='ascending',
        ...
        for cnt, q_idx in enumerate(tqdm.tqdm(query_indices)):
            ...
            q_path = query_info['img_paths']
            q_pattern = re.compile(r'([-\d]+)_c([-\d]+)')
            q_pid, cid = map(int, q_pattern.search(q_path).groups())
            ...
            ax.imshow(query_img)
            ax.set_title('{:d}/cam{}'.format(q_pid, cid))
            ax.axis("off")
            for i in range(max_rank):
                ...
                path = gallery_info['img_paths']
                print(path)
                pattern = re.compile(r'([-\d]+)_c([-\d]+)')
                pid, cid = map(int, pattern.search(path).groups())
                # print(gallery_info)
                ...
                ax.set_title(f'{pid}/{label}/cam{cid}')
                ax.axis("off")
            # if actmap:
            #     act_outputs = []
            #
